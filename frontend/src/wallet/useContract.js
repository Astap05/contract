...
export function useContract({ client, contractAddress, address }) {
    const [logs, setLogs] = useState([]);
    const push = (line) => setLogs(prev => [...prev.slice(-500), line]);
    const appendLog = (line) => push(line);
    ...
    const getTaskInfo = (participant) => run(async () => {
        if (!client || !contractAddress)
            throw new Error('Нет клиента/адреса контракта');
    ...
    const register = () => run(async () => {
        if (!client || !contractAddress || !address)
            throw new Error('Нет клиента/адреса контракта/кошелька');
    ...
    const submitTask = (description) => run(async () => {
        if (!client || !contractAddress || !address)
            throw new Error('Нет клиента/адреса контракта/кошелька');
    ...
    const validateTask = (participant) => run(async () => {
        if (!client || !contractAddress || !address)
            throw new Error('Нет клиента/адреса контракта/кошелька');
    ...
    const approveDistribution = (participant, amount) => run(async () => {
        if (!client || !contractAddress || !address)
            throw new Error('Нет клиента/адреса контракта/кошелька');
    ...
    const deposit = (denom, amount) => run(async () => {
        if (!client || !contractAddress || !address)
            throw new Error('Нет клиента/адреса контракта/кошелька');
        const msg = { deposit: {} };
